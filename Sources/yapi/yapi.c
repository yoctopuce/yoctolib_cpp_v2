...
static YRETCODE yapiSetSSLCertificate_internal(const char* certificate, const char* privatekey, char* errmsg)
{
    if (!yContext) {
...
#ifndef NO_YSSL
    int res = yTcpSetCertificateSSL(certificate, privatekey, errmsg);
    if (YISERR(res)) {
        return (YRETCODE)res;
    }
    return YAPI_SUCCESS;
...
#pragma warn . 8066
    ...
    if (certificate || privatekey) {
        int res = yapiSetSSLCertificate_internal(certificate, privatekey, errmsg);
        if (res < 0) {
            yapiFreeAPI();
        }
        return res;
...
YRETCODE YAPI_FUNCTION_EXPORT yapiSetSSLCertificate(const char* certfile, const char* keyfile, char* errmsg)
{
    ...
    res = yapiSetSSLCertificate_internal(certfile, keyfile, errmsg);
    YDLL_CALL_LEAVE(res);
    return res;
}
